...
class MCProtectIpstore_V602 {
	...
	function __construct($storage_type = MCProtectIpstore_V602::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V602::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V602();
		} else {
			$this->storage = new MCProtectIpstoreDB_V602();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V602::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V602::STORAGE_TYPE_DB) {
